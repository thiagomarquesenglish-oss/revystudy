...
export function validateDeckGeneration(value, previous) {
  const items = value?.situations;
  const fail = () => { throw new Error('A IA não retornou 20 situações válidas no formato pedido. Tente gerar novamente.'); };
  if (!Array.isArray(items) || items.length !== 20) return fail();
  ...
  if (bridges !== (previous.length ? 10 : 0)) return fail();
  return result;
}
