...
export default function CurriculumPanel(){
  ...
  return <section className="space-y-5" aria-label="Progresso do currículo">
    ...
    <div><h3 className="font-semibold mb-3">Seu caminho</h3><ol className="space-y-2">{progress.units.map(stage=>{const state=stageLearningState(stage);const label=stage.earned&&!stage.mastered?'✓ Concluída':state==='preparing'?`${stage.count} de ${stage.targetContent} conteúdos`:stateLabel[state];return <li key={stage.stage} className="flex items-center justify-between gap-3 rounded-xl border border-border px-4 py-3"><span className="text-sm">{stage.stage}. {stage.title}</span><span className="text-xs text-muted-foreground">{label}</span></li>})}</ol></div>
