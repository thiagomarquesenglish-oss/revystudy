...
export default function SituationAddForm({ deckId }: { deckId: string }) {
  ...
  const ready=!!english.trim()&&!!image&&(!!audio||audioId!=='none');
  ...
  return <form id="situation-form" onSubmit={save} className="space-y-5">
    ...
    <section className="rounded-2xl border border-border bg-card p-4 space-y-4"><div className="flex gap-2"><MessageSquareText className="h-5 w-5 text-primary"/><h2 className="font-bold">2. Frases</h2></div><div><Label htmlFor="english">Frase em inglês</Label><Input id="english" lang="en" value={english} onChange={e=>setEnglish(e.target.value)} placeholder="The kitchen is over here."/><p className="mt-1 text-xs text-muted-foreground">Esta mesma frase será usada para corrigir o ditado.</p></div><div><Label htmlFor="context">Contexto em inglês (opcional)</Label><Input id="context" lang="en" value={context} onChange={e=>setContext(e.target.value)} placeholder="You are showing a guest around your house."/></div><div><Label htmlFor="portuguese">Frase em português (opcional)</Label><Input id="portuguese" value={portuguese} onChange={e=>setPortuguese(e.target.value)} placeholder="A cozinha fica aqui."/><p className="mt-1 text-xs text-muted-foreground">Serve apenas como apoio temporário e desaparece com seu progresso.</p></div></section>
    ...
    <Button className="w-full h-12" disabled={!ready||saving}>{saving?'Enviando e criando…':'Criar situação'}</Button>{!ready&&<p className="text-xs text-center text-muted-foreground">Adicione uma imagem, um áudio e a frase em inglês.</p>}
  </form>;
}
